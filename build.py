...
<div class="controls" role="group" aria-label="Background controls">
  <p class="lattice-legend">Filled: infected · Hollow: healthy<br>Grid neighbors · Synchronous rounds</p>
  ...
  <button type="button" class="ctl" id="lattice-toggle" hidden aria-label="Show the exact bootstrap lattice" aria-pressed="false" title="Exact lattice: filled sites are infected; hollow sites are healthy."><span class="ctl-ico" aria-hidden="true"><svg viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.1"><path d="M6 3v14M14 3v14M3 6h14M3 14h14"/></svg></span><span class="ctl-txt">Lattice</span></button>
...
<script src="assets/coffee-surface.js"></script>
...
# Abstracts are reviewed, source-linked snapshots; builds never need the network.
abstracts = load("abstracts.json")
import hashlib
for key, entry in abstracts.items():
    if hashlib.sha256(entry["text"].encode()).hexdigest() != entry["sha256"]:
        raise ValueError("Retypeset edited abstract: " + key)
with open(os.path.join(HERE, "assets", "abstracts.js"), "w", encoding="utf-8") as f:
    f.write("/*! Generated by build.py from data/abstracts.json. */\nwindow.ABSTRACTS = ")
    f.write(json.dumps(abstracts, ensure_ascii=False, indent=1).replace("<", "\\u003c"))
    f.write(";\n")
print("wrote assets/abstracts.js (%d abstracts)" % len(abstracts))
