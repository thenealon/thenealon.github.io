...
(function () {
  ...
  function labelBg(m) {
    ...
    var latticeControl = document.getElementById('lattice-toggle');
    if (latticeControl) { latticeControl.hidden = !perc; }
  ...
  /* Optional exact state: view-only, including while paused. */
  var latticeBtn = document.getElementById('lattice-toggle');
  if (latticeBtn && window.tidalGraph) {
    latticeBtn.addEventListener('click', function () {
      var shown = !window.tidalGraph.getLattice();
      window.tidalGraph.setLattice(shown);
      latticeBtn.setAttribute('aria-pressed', String(shown));
      latticeBtn.setAttribute('aria-label', shown ? 'Hide the exact bootstrap lattice' : 'Show the exact bootstrap lattice');
      root.setAttribute('data-lattice', shown ? 'on' : 'off');
    });
  }
